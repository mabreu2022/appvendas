...
procedure TfrmMain.AtualizaEmpresas;
const
  cMETHOD = 'TfrmMain.AtualizaEmpresas';
var
  vQryAtua: TFDQuery;
begin
  try
    try
      dm._ConexaoFB.ExecComand('delete from ADM_EMPRESA');

      dm.qryAdmEmpresasFB.Close;
      dm.qryAdmEmpresasFB.Open;
      dm.qryAdmEmpresasFB.DisableControls;

      vQryAtua := dm._ConexaoCli.GetQuery('select * from V_ADM_EMPRESA');

      while not vQryAtua.Eof do
      begin
        dm.qryAdmEmpresasFB.Insert;
        dm.qryAdmEmpresasFBCODEMP.AsInteger          := vQryAtua.FieldByName('CODEMP').AsInteger;
        dm.qryAdmEmpresasFBNOME.AsString             := vQryAtua.FieldByName('NOME').AsString;
        dm.qryAdmEmpresasFBNOMEFANTASIA.AsString     := vQryAtua.FieldByName('NOMEFANTASIA').AsString;
        dm.qryAdmEmpresasFBENDERTPLOGRAD.AsString    := vQryAtua.FieldByName('ENDERTPLOGRAD').AsString;
        dm.qryAdmEmpresasFBENDERLOGRAD.AsString      := vQryAtua.FieldByName('ENDERLOGRAD').AsString;
        dm.qryAdmEmpresasFBENDERNUMLOGRAD.AsString   := vQryAtua.FieldByName('ENDERNUMLOGRAD').AsString;
        dm.qryAdmEmpresasFBENDERCOMPLEMENTO.AsString := vQryAtua.FieldByName('ENDERCOMPLEMENTO').AsString;
        dm.qryAdmEmpresasFBENDERBAIRRO.AsString      := vQryAtua.FieldByName('ENDERBAIRRO').AsString;
        dm.qryAdmEmpresasFBCODCIDADE.AsInteger       := vQryAtua.FieldByName('CODCIDADE').AsInteger;
        dm.qryAdmEmpresasFBNUMTELEFONE.AsString      := vQryAtua.FieldByName('NUMTELEFONE').AsString;
        dm.qryAdmEmpresasFBUSUARIO.AsInteger         := vQryAtua.FieldByName('USUARIO').AsInteger;
        dm.qryAdmEmpresasFBDATAATU.AsDateTime        := vQryAtua.FieldByName('DATAATU').AsDateTime;
        dm.qryAdmEmpresasFBHORAATU.AsString          := vQryAtua.FieldByName('HORAATU').AsString;
        dm.qryAdmEmpresasFBDOCFISCAL.AsString        := vQryAtua.FieldByName('DOCFISCAL').AsString;
        dm.qryAdmEmpresasFBDOCFISCAL.AsString        := vQryAtua.FieldByName('DOCFISCAL').AsString;
        dm.qryAdmEmpresasFB.Post;
      ...
      on E : Exception do begin
        uLogger.Instance.Erro(cMETHOD, 'Erro ao atualizar Empresas', '', E);
      end;
    end;
  finally
    vQryAtua.Free;
...
begin
  try
    try
      ...
      dm.qryClientesFB.EnableControls;
  ...
  try
    try
      ...
      begin
        ...
        dm.qryProdutosFBCODEMP.AsInteger       := dm.qryProdutosCliCODEMP.AsInteger;
        dm.qryProdutosFBCODIGO.AsString        := dm.qryProdutosCliCODPROD.AsString;
        dm.qryProdutosFBDESCRICAO.AsString     := dm.qryProdutosCliDESCCOMPLETA.AsString;
        dm.qryProdutosFBDESCRICAO_RED.AsString := dm.qryProdutosCliDESCRICAO.Value;
        dm.qryProdutosFBBARRAS.AsString        := dm.qryProdutosCliCODBAR.Value;
        dm.qryProdutosFBUNIDADE.AsString       := dm.qryProdutosCliUNIDMEDIDA.AsString;
        dm.qryProdutosFBPRECO_VENDA.AsFloat    := dm.qryProdutosCliVLVENDA.AsFloat;
        dm.qryProdutosFBPRECO_MIN.AsFloat      := dm.qryProdutosCliVLVENDAMIN.AsFloat;
        dm.qryProdutosFBPRECO_ATACADO.Value    := dm.qryProdutosCliVLVENDAATACADO.AsFloat;
        dm.qryProdutosFBPRECO_COMPRA.Value     := dm.qryProdutosCliVLCTOULTCOMP.AsFloat;
        dm.qryProdutosFBDAT_ALTERADO.AsDateTime:= dm.qryProdutosCliDTULTALT.AsDateTime;
