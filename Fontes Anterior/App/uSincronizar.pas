...
uses
  ...
  REST.Types, FireDAC.Comp.Client;
...
type
  ...
  public
    ...
    class function IncReceber(cod_pagto: string; cod_emp, cod_cliente, num_fat, num_parc: integer): TRecSincronizar;
...
uses
  uBanco, uFuncoes, uReceberRecCmd, uConverter;
...
class function TClsSincronizar.IncReceber(cod_pagto: string; cod_emp, cod_cliente, num_fat, num_parc: integer): TRecSincronizar;
var
  cmd: string;
begin
  try
    cmd := TClsReceberRecCmd.GetInsert(cod_pagto, cod_emp, cod_cliente, num_fat, num_parc);
    dmBanco.ExecComando(cmd);
    result.Qtde := 1;
  except
    on E : Exception do begin
      result.Erro := true;
      result.Mensagem := 'Erro ao incluir receber pend !!!' + E.Message;
      raise Exception.Create(result.Mensagem);
    end;
  end;
end;
...
begin
  try
    ...
    on E : Exception do begin
      result.Erro := true;
      result.Mensagem := 'Erro ao atualizar receber pend !!!' + E.Message;
      raise Exception.Create(result.Mensagem);
    end;
  end;
end;
