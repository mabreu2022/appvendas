...
type
  ...
  public
    ...
    class function IncReceber(cod_pagto: string; cod_emp, cod_cli, num_fat, num_par: integer): TRecSincronizar;
    class function UpdReceber(cod_pagto: string): TRecSincronizar;
    class function RemReceberPend(): TRecSincronizar;
    class function IncReceberPag(cod_pagto: string; nro_pagto, tip_doc, cod_mod: Integer; vlr_doc: Real; dat_doc: TDateTime; nro_doc, nro_ban, nro_cta: integer): TRecSincronizar;
...
begin
  try
    ...
    dmBanco.qryAux.sql.Add('select * from TB_PEDIDO p ' + 'inner join TB_ITENS_PEDIDO i on p.codigo = i.cod_pedido ' + 'where p.CODIGO = :codigo');
...
class function TClsSincronizar.IncReceber(cod_pagto: string; cod_emp, cod_cli, num_fat, num_par: integer): TRecSincronizar;
var
  cmd: string;
begin
  try
    cmd := TClsReceberRecCmd.GetInsert(cod_pagto, cod_emp, cod_cli, num_fat, num_par);
...
class function TClsSincronizar.IncReceberPag(cod_pagto: string; nro_pagto, tip_doc, cod_mod: Integer; vlr_doc: Real; dat_doc: TDateTime; nro_doc, nro_ban, nro_cta: integer): TRecSincronizar;
var
  cmd: string;
begin
  try
    cmd := TClsReceberRecCmd.GetInsertPag(cod_pagto, nro_pagto, tip_doc, cod_mod, vlr_doc, dat_doc, nro_doc, nro_ban, nro_cta);
    dmBanco.ExecComando(cmd);
    result.Qtde := 1;
  except
    on E: Exception do
