...
{$METHODINFO ON}
  ...
  TReqServerMethods = (
    ...
    trGetBanco, trGetConta, trGetMoeda, trGetRecibo,  trGetEmpresa,
    trSetPedido,
...
const
  ...
  CReqServerMethods : array [TReqServerMethods] of string = (
    ...
    '/GetBanco', '/GetConta', '/GetMoeda', '/GetRecibo', '/GetEmpresa',
    '/SetPedido',
...
begin
  ...
  dm.qryPedidosFBCODEMP.Value := dm.memPedidos.FieldByName('CODEMP').Value;
...
function TServerMethods1.GetEmpresa: String;
var
  vQryAux: TFDQuery;
Begin
  vQryAux := dm._ConexaoFB.GetQuery('select * from ADM_EMPRESA');
  Result := DatasetToJson(vQryAux);
  vQryAux.Free;
end;
