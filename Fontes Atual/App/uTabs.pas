...
type
  TfrmTabs = class(TForm)
    ...
    procedure Layout60Click(Sender: TObject);
    procedure Layout61Click(Sender: TObject);
  private
...
procedure TfrmTabs.GravarPedido;
var
  total: currency;
  i: integer;
  cod: string;
...
procedure TfrmTabs.AtualizarPedido;
var
  total: currency;
  i: integer;
begin
  ...
  dmBanco.qryAux.Close;
  dmBanco.qryAux.sql.Clear;
  dmBanco.qryAux.sql.Add(' UPDATE TB_PEDIDO SET ');
  dmBanco.qryAux.sql.Add(' DATA_PEDIDO = :DATA_PEDIDO, ');
  dmBanco.qryAux.sql.Add(' VENDEDOR = :VENDEDOR, ');
  dmBanco.qryAux.sql.Add(' TOTAL_PEDIDO = :TOTAL_PEDIDO, ');
  dmBanco.qryAux.sql.Add(' OBS = :OBS, ');
  dmBanco.qryAux.sql.Add(' CLIENTE = :CLIENTE, ');
  dmBanco.qryAux.sql.Add(' COD_PAGTO = :COD_PAGTO, ');
  dmBanco.qryAux.sql.Add(' CODEMP = :CODEMP ');
  dmBanco.qryAux.sql.Add(' WHERE CODIGO = :CODIGO ');
  dmBanco.qryAux.sql.Add(' AND TIPO = :TIPO ');
  dmBanco.qryAux.ParamByName('CODIGO').Value := vCodigo;
  dmBanco.qryAux.ParamByName('TIPO').Value := 'P';
  dmBanco.qryAux.ParamByName('DATA_PEDIDO').AsDateTime := now;
  dmBanco.qryAux.ParamByName('VENDEDOR').Value := StrToInt(copy(ComboBoxVendedor.Selected.Text, 1, Pos('-', ComboBoxVendedor.Selected.Text) - 1));
  dmBanco.qryAux.ParamByName('TOTAL_PEDIDO').Value := total;
  dmBanco.qryAux.ParamByName('OBS').Value := '';
  dmBanco.qryAux.ParamByName('CLIENTE').Value := Label6.Tag;
  dmBanco.qryAux.ParamByName('COD_PAGTO').Value := StrToInt(copy(ComboBoxPagto.Selected.Text, 1, Pos('-', ComboBoxPagto.Selected.Text) - 1));
  dmBanco.qryAux.ParamByName('CODEMP').Value := TAtualizaBD.LerINI('Usuario', 'CodEmp');
  dmBanco.qryAux.ExecSQL;
  ...
  dmBanco.qryAux.Close;
  dmBanco.qryAux.sql.Clear;
  dmBanco.qryAux.sql.Add(' delete from TB_ITENS_PEDIDO ');
  dmBanco.qryAux.sql.Add(' where COD_PEDIDO = :COD_PEDIDO ');
  dmBanco.qryAux.ParamByName('COD_PEDIDO').Value := vCodigo;
  dmBanco.qryAux.ExecSQL;
...
var
  vItem: TListViewItem;
begin
  vItem := TListViewItem(AItem);
  vCodigo := vItem.Data['Text1'].AsString;
...
procedure TfrmTabs.DeletarPedido;
var
  txt: TListItemText;
begin
  txt := TListItemText(ListViewPedido.Items[ListViewPedido.ItemIndex].Objects.FindDrawable('Text1'));
...
procedure TfrmTabs.Layout60Click(Sender: TObject);
begin
  CloseMenu(recMenu, recSubMenu);
  DeletarPedido;
end;
...
begin
  ...
  dmBanco.qryPadrao.SQL.Add(' SELECT DISTINCT ');
  ...
  dmBanco.qryPadrao.ParamByName('CODIGO').AsString := vCodigo;
...
//  Memo1.Lines.Add('Operacion: ' + dmBanco.qryPadrao.FieldByName('TIPO').AsString);
  ...
  dmBanco.qryPadrao.First;
  while not dmBanco.qryPadrao.Eof do
  begin
    ...
    Memo1.Lines.Add(FormatFloat('###,###,##0.00', StrToFloatDef(dmBanco.qryPadrao.FieldByName('QTD').AsString, 0)) + ' ' + FormatFloat('###,###,##0.00', StrToFloatDef(dmBanco.qryPadrao.FieldByName('VALOR_DESCONTO').AsString, 0)) + ' ' + FormatFloat('###,###,##0.00', StrToFloatDef(dmBanco.qryPadrao.FieldByName('VALOR_UNITARIO').AsString, 0)) + ' ' + FormatFloat('###,###,##0.00', StrToFloatDef(dmBanco.qryPadrao.FieldByName('VALOR_TOTAL').AsString, 0)));
    Memo1.Lines.Add('');
    dmBanco.qryPadrao.Next;
  end;
...
procedure TfrmTabs.PreencherCamposEdicao;
begin
  dmBanco.qryPadrao.Close;
  dmBanco.qryPadrao.SQL.Clear;
  dmBanco.qryPadrao.SQL.Add(' SELECT DISTINCT ');
  ...
  dmBanco.qryPadrao.SQL.Add('     VE.NOME NM_VENDEDOR, ');
  dmBanco.qryPadrao.SQL.Add('     COALESCE((SELECT SUM(VALOR_DESCONTO) DESCONTO FROM TB_ITENS_PEDIDO WHERE COD_PEDIDO = :CODIGO), 0) DESCONTO, ');
  dmBanco.qryPadrao.SQL.Add('     CP.DESCRICAO DESCRICAO_PAGAMENTO ');
  dmBanco.qryPadrao.SQL.Add(' FROM TB_ITENS_PEDIDO IP ');
  dmBanco.qryPadrao.SQL.Add(' LEFT JOIN TB_PRODUTO P ON P.CODIGO = IP.COD_PRODUTO ');
  dmBanco.qryPadrao.SQL.Add(' LEFT JOIN TB_PEDIDO PE ON PE.CODIGO = IP.COD_PEDIDO ');
  dmBanco.qryPadrao.SQL.Add(' LEFT JOIN ADM_EMPRESA EM ON EM.CODEMP = PE.CODEMP ');
  dmBanco.qryPadrao.SQL.Add(' LEFT JOIN TB_CLIENTE CL ON CL.CODIGO = PE.CLIENTE ');
  dmBanco.qryPadrao.SQL.Add(' LEFT JOIN TB_VENDEDOR VE ON VE.CODIGO = PE.VENDEDOR ');
  dmBanco.qryPadrao.SQL.Add(' LEFT JOIN TB_COND_PAGTO CP ON CP.CODIGO = PE.COD_PAGTO ');
  dmBanco.qryPadrao.SQL.Add(' WHERE COD_PEDIDO = :CODIGO ');
  dmBanco.qryPadrao.ParamByName('CODIGO').AsString := vCodigo;
  dmBanco.qryPadrao.Open();

  Label6.Text := dmBanco.qryPadrao.FieldByName('NM_CLIENTE').AsString;
  Label6.Tag := dmBanco.qryPadrao.FieldByName('CLIENTE').AsInteger;
  ComboBoxVendedor.ItemIndex := ComboBoxVendedor.Items.IndexOf(dmBanco.qryPadrao.FieldByName('VENDEDOR').AsString + '-' + dmBanco.qryPadrao.FieldByName('NM_VENDEDOR').AsString);
  ComboBoxPagto.ItemIndex := ComboBoxPagto.Items.IndexOf(dmBanco.qryPadrao.FieldByName('COD_PAGTO').AsString + '-' + dmBanco.qryPadrao.FieldByName('DESCRICAO_PAGAMENTO').AsString);

  while not dmBanco.qryPadrao.Eof do
  begin
    dmBanco.memitens.Append;
    dmBanco.memitensCOD_PRODUTO.Value := dmBanco.qryPadrao.FieldByName('COD_PRODUTO').AsString;
    dmBanco.memitensDESCRICAO.Value := dmBanco.qryPadrao.FieldByName('DESCRICAO').AsString;
    dmBanco.memitensQTD.Value := dmBanco.qryPadrao.FieldByName('QTD').AsInteger;
    dmBanco.memitensVLR_UNITARIO.Value := dmBanco.qryPadrao.FieldByName('VALOR_UNITARIO').AsFloat;
    dmBanco.memitensVLR_DESCONTO.Value := dmBanco.qryPadrao.FieldByName('VALOR_DESCONTO').AsFloat;
    dmBanco.memitensVLR_LIQUIDO.Value := dmBanco.qryPadrao.FieldByName('VALOR_UNITARIO').AsFloat - dmBanco.qryPadrao.FieldByName('VALOR_DESCONTO').AsFloat;
    dmBanco.memitensVLR_TOTAL.Value := dmBanco.qryPadrao.FieldByName('QTD').AsInteger * dmBanco.memitensVLR_LIQUIDO.Value;
    dmBanco.memitens.Post;
    with ListViewItemPedido.Items.Add do
    begin
      Data['Text1'] := dmBanco.qryPadrao.FieldByName('COD_PRODUTO').AsString + ' - ' + dmBanco.qryPadrao.FieldByName('DESCRICAO').AsString;
      Data['Text2'] := dmBanco.memitensQTD.AsString + ' X $ ' + formatfloat('##,###,###0.00', dmBanco.memitensVLR_UNITARIO.Value);
      Data['Text3'] := '$ ' + formatfloat('##,###,###0.00', dmBanco.memitensVLR_TOTAL.Value);
    end;
    dmBanco.qryPadrao.Next;
...
procedure TfrmTabs.HabilitarDados;
begin
  dmBanco.qryProdutos.Close;
  dmBanco.qryProdutos.sql.Clear;
...
procedure TfrmTabs.Layout61Click(Sender: TObject);
begin
  inAtualizar := True;
  CloseMenu(recMenu, recSubMenu);
  HabilitarDados;
