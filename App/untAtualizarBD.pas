...
type
  ...
  public
    ...
    class procedure AtualizarBanco(Conexao: TFDConnection = nil);
...
class function TAtualizaBD.GetConexao: TFDConnection;
begin
  if Assigned(FConexao) then
    Result := FConexao
  else if Assigned(dmBanco) then
    Result := dmBanco.Conexao
  else
    Result := nil;
end;

class procedure TAtualizaBD.AtualizarBanco(Conexao: TFDConnection);
begin
...
begin
  ...
  bFDQuery.Connection := GetConexao();
...
begin
  ...
  if Assigned(GetConexao()) then
    GetConexao().ExecSQL(xSql);
